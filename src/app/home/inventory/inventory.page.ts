    ...
    this.dev_data_arr.forEach((value) => {
      ...
      if (value.dynamicVolume >= 65) {
        ...
        value.dynamicMessage =
          "Arrrrrrghhhh, its time to fill up, its running empty";
      }
    });
  }
    ...
    try {
      firebase.auth().onAuthStateChanged((user: firebase.User) => {
        if (user) {
          ...
          this.crud_service
            .read_linked_devices(this.userdata["uid"])
            .subscribe((data) => {
              console.log();
              if (data.data()["linked_devices"] != null) {
                ...
                this.devices_list.forEach((value) => {
                  this.crud_service
                    .read_data(value["id"])
                    .subscribe((linkdata) => {
                      var temp: device_data = {
                        clientsId: linkdata.data()["Client"],
